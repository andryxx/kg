...
const outputFile = "./pharmobkg/chem/chem.mapping.csv";
...
(async () => {
    ...
    for (const mappingElement of mapping) {
        ...
        if (!mappingElement["UMLS"] && mappingElement["NDF-RT"]) {
            const vocId = mappingElement["NDF-RT"];
            if (medrtUmlsMapping[vocId]) {
                mappingElement["UMLS"] = medrtUmlsMapping[vocId];
                counter.add("mapped from NDF-RT");
            } else {
                counter.add("mapping not found in NDF-RT");
            }
        }
