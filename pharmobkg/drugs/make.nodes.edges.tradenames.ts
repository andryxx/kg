...
const outputFile = "./pharmobkg/drugs/nodes.drugs.tradenames.csv";
const edgesCsvFile = "./pharmobkg/drugs/edges.drugs.tradenames.csv";
...
(async () => {
    ...
    for (const dataRow of dataSet) {
        ...
        if (!tNames) {
            counter.add("drugs without tradenames");
            continue;
        }
        counter.add("drugs with tradenames");
        const drugNodeId = ["pgkb", id.toLowerCase()].join("_");

        let gCount = 1;
        for (const tradename of tNames.replace(/"/g, "").split(", ")){
            ...
            await nodes.write([
                ...
                id, // "node_code:String",
                tradename, // `"node_name:String"`,
                "pharmgkb", // "node_source_1:String",
                // // ?	node_source_2:String
                "atom", // "type:String", // ?	
                uploadDate, // "uploaded_at:Date", // "YYYY-MM-DD"
            ]);
            counter.add("tradenames added");
            await logEdgesPair({ from: drugNodeId, to: tradeNameId, directLbl: "Has tradename", invertedLbl: "Tradename of"}, edges);
        }
    }
